  ...
  describe 'トップ画面のテスト' do
    ...
    context '表示内容の確認' do
      ...
      it 'Sign upリンクが表示される: 緑色のボタンの表示が「Sign up」である', spec_category: "ルーティング・URL設定の理解(ログイン状況に合わせた応用)" do
        sign_up_link = find_link('Sign up').text
  ...
  describe 'ヘッダーのテスト: ログインしていない場合' do
    ...
    context '表示内容の確認' do
      ...
      it 'Aboutリンクが表示される: 左上から3番目のリンクが「About」である', spec_category: "CRUD機能に対するコントローラの処理と流れ(ログイン状況を意識した応用)" do
        ...
        expect(about_link).to have_text("homes")
      end
      it 'Sign upリンクが表示される: 左上から4番目のリンクが「Sign up」である', spec_category: "CRUD機能に対するコントローラの処理と流れ(ログイン状況を意識した応用)" do
        ...
        expect(signup_link).to have_text("Sign up")
      end
